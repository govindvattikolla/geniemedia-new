  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 sm:p-8 max-w-sm w-full shadow-2xl border border-green-100 animate-scaleIn">
        ...
        <p className="text-sm text-gray-500 text-center mb-1">
          This will make the blog live for all users:
        </p>
        <p className="text-sm font-semibold text-gray-800 text-center mb-6 line-clamp-2 px-2">
          "{blog.title}"
        </p>
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow">
        {isDraft && (
          <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 mb-3">
            ...
            <p className="text-[10px] sm:text-xs text-amber-700 font-semibold">
              Hidden from public · Admin view only
            </p>
          </div>
        )}
...
export default function AdminBlogs() {
      ...
      if (allRes.ok) {
        ...
        setBlogs(
          Array.isArray(pubData)
            ? pubData.sort((a, b) => b.createdAt - a.createdAt)
            : []
        );
      }
    } catch (err) {
  ...
  const handleCategorySelect = (cat) => {
    setForm((prev) => ({
      ...prev,
      category: cat,
      permalink: permalinkManual
        ? prev.permalink
        : buildPermalink(prev.title, cat),
    }));
  };
    ...
    <main className="w-full min-h-screen bg-[#F7F6F3] overflow-x-hidden font-sans">
      ...
      <header className="relative min-h-[40vh] sm:min-h-[55vh] flex items-center justify-center overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=1400&q=80')",
          }}
        />
          ...
          <div className="flex items-center justify-between">
            <div className="flex overflow-x-auto no-scrollbar">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 sm:px-5 py-3.5 sm:py-4 text-xs sm:text-sm font-bold border-b-2 transition-all whitespace-nowrap ${
                  activeTab === "published"
            ...
            <button
              ...
              className="sm:hidden p-2 hover:bg-gray-100 rounded-lg shrink-0"
            >
        ...
        <section className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-10">
          {activeTab === "drafts" && (
            <div className="mb-5 sm:mb-6 bg-amber-50 border-2 border-amber-200 rounded-2xl px-4 sm:px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
              <Lock
                size={18}
                className="text-amber-700 shrink-0 mt-0.5 sm:mt-0"
              />
              <div className="flex-1">
                <p className="text-sm font-bold text-amber-800">
                  🔒 Drafts are ADMIN-ONLY — completely hidden from all users
                </p>
                <p className="text-xs text-amber-600 mt-1 leading-relaxed">
                  Users <strong>cannot see drafts</strong> at all. Only when you
                  click <strong>"Publish"</strong> does a blog become visible to
                  the public.
                </p>
              </div>
              {draftBlogs.length > 0 && (
          ...
          <p className="text-xs sm:text-sm text-gray-500 mb-4 font-medium">
            Showing{" "}
            <strong className="text-gray-800">{filteredBlogs.length}</strong> of{" "}
          ...
          {dbLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 py-16 sm:py-20 text-center px-4">
              ...
              <p className="text-xs sm:text-sm text-gray-400 mb-6">
                {activeTab === "drafts"
                  ? "Save a blog as draft — it stays hidden from users until you publish."
                  : blogs.length === 0
      ...
      {activeTab === "create" && (
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <form
              onSubmit={(e) => handleSubmit(e, saveAsDraft)}
              className="px-4 sm:px-8 py-6 sm:py-8 space-y-5 sm:space-y-6"
            >
              ...
              <Field
                ...
                hint={
                  permalinkManual
                    ...
                    : "Auto-generated from category + title · click to edit manually"
                }
              >
                  ...
                  {permalinkManual && !editingId && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setForm((p) => ({
                          ...p,
                          permalink: buildPermalink(p.title, p.category),
                        }));
                      }}
                ...
                {form.permalink && (
                  <p className="text-[11px] text-gray-400 font-mono mt-1.5 pl-1">
                    ...
                    <span className="text-[#6B4A2D]">
                      yourdomain.com/blog/{form.permalink}
                    </span>
                  </p>
                )}
              </Field>
              ...
              <Field label="Featured Image" icon={Image}>
                ...
                >
                  <div className="w-10 h-10 rounded-xl bg-gray-100 group-hover:bg-[#6B4A2D]/10 flex items-center justify-center transition">
                    <Image
                      size={18}
                      className="text-gray-400 group-hover:text-[#6B4A2D] transition"
                    />
                  </div>
                  <div className="text-center">
                ...
                <div className="mt-2.5 bg-blue-50 border border-blue-200 rounded-xl overflow-hidden">
                  <div className="px-4 py-2 bg-blue-100 border-b border-blue-200">
                    <p className="text-xs font-bold text-blue-900 flex items-center gap-1.5">
                      <Image size={12} /> 📐 Required Image Specifications
                    </p>
                  </div>
                  <div className="px-4 py-3 grid grid-cols-3 gap-3 text-xs">
                    {[
                      ...
                      <div
                        key={label}
                        className="bg-white rounded-lg p-2.5 border border-blue-100 text-center"
                      >
                ...
                {form.imagePreview && (
                  <div className="mt-3">
                    <p className="text-xs font-semibold text-gray-500 mb-1.5 flex items-center gap-1.5">
                      <CheckCircle size={11} className="text-green-500" /> Image
                      Preview — 16:9 ratio
                    </p>
                    <div
              ...
              <div className="rounded-xl border-2 border-dashed border-gray-200 p-4 bg-gray-50 space-y-3">
                ...
                <div className="flex items-start gap-2.5">
                  <div className="shrink-0 w-7 h-7 bg-amber-100 rounded-lg flex items-center justify-center mt-0.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-amber-700 leading-relaxed mt-0.5">
                      Blog is saved <strong>only for you</strong>. Users{" "}
                      <strong>cannot see it</strong> until you explicitly click
                      Publish.
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-2.5">
                  <div className="shrink-0 w-7 h-7 bg-green-100 rounded-lg flex items-center justify-center mt-0.5">
                    <Globe size={13} className="text-green-700" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-green-800">
                      Publish Now to Users
                    </p>
                    <p className="text-[11px] text-green-700 leading-relaxed mt-0.5">
                      Blog goes <strong>live immediately</strong>. All visitors
                      can find and read it right away.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-5 sm:mt-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-5">
            <h4 className="text-xs sm:text-sm font-bold text-amber-800 mb-3">
              💡 Quick Tips
            </h4>
            <ul className="text-[11px] sm:text-xs text-amber-700 space-y-1.5">
              <li className="flex items-start gap-2">
                <span>→</span> Permalink auto-generates as{" "}
                <strong>category/blog-title</strong> — select category first for
                best results
              </li>
              <li className="flex items-start gap-2">
                <span>→</span> You can manually edit the permalink — click Reset
                to regenerate
              </li>
              <li className="flex items-start gap-2">
                <span>→</span> Keep meta description under 160 chars for best
                SEO
              </li>
              <li className="flex items-start gap-2">
                <span>→</span> Press{" "}
                <kbd className="bg-amber-100 px-1 rounded font-mono">Enter</kbd>{" "}
                or <kbd className="bg-amber-100 px-1 rounded font-mono">,</kbd>{" "}
                to add keyword tags
              </li>
              <li className="flex items-start gap-2">
                <span>→</span> Best image:{" "}
                <strong>1200×675px · JPG/WebP · 16:9 · max 500KB</strong>
              </li>
              <li className="flex items-start gap-2">
                <span>→</span> Drafts are 100% hidden — users only see blogs you
                explicitly Publish
              </li>
            </ul>
          </div>
